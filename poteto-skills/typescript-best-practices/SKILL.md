...
Type-system discipline comes first: make illegal states unrepresentable, brand semantic primitives, parse external data at boundaries, never lie to the compiler, exhaust every variant, and derive types from authoritative schemas.
...
| Boundary validation | Parse where data crosses in, into a named domain type. `Record<string, unknown>` (however spelled) stops at that parse. Trust types inside. Concentrate guards at system boundaries (network, config, storage, external APIs) and keep internal logic free of defensive re-checks. |
