---
name: why
description: "Use for 'why does X work this way', 'why we picked Y', 'why is this code here', design rationale, regressions, postmortems, or data-backed thresholds. Discovers available MCPs and queries each evidence category (source control, issue tracker, long-form docs, real-time chat, infrastructure observability, error tracking, product analytics warehouse) in parallel, then returns a cited read on decisions and tradeoffs. Use how for runtime behavior. Not for questions about the agent's own recent edits in this session ('why did you change X'); answer those directly."
---
...
Spawn subagents with the `Agent` tool, `subagent_type: general-purpose`. Leave `model` unset so subagents inherit the session model. Subagents inherit the session's MCP tools, so every investigator can reach the MCP it owns.
...
If the target is vague ("why do we do it this way?" with no clear referent), make your best guess from conversation context (files open in the IDE, recent edits, the current selection, what was just discussed). State your interpretation briefly so the user can redirect if you're off, then proceed.
...
Before spawning investigators, list the MCP servers available in this Claude Code session. MCP tools are named `mcp__<server>__<tool>`. Check both the loaded tools and the deferred tools named in system reminders (load a deferred tool's schema with `ToolSearch` before calling it). Server instructions in the system prompt describe what each server is for.
...
- `subagent_type`: `general-purpose` (not `Explore`: investigators need `gh` and MCP calls, and their judgment matters more than speed)
- `run_in_background`: `false`
- Investigators must not write files, commit, or change external state. Say so in each prompt.
...
- `run_in_background`: `false`
- The synthesizer spot-verifies citations, which can require `gh` or MCP calls. It must not write files or change external state.
