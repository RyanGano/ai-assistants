...
- **Drive:** how can an agent interact with it programmatically? Existing harnesses first — Playwright/Cypress specs, expect scripts, PTY helpers, curl-able endpoints, a debug port. Only then pick a generic recipe: for web, the session's browser tools (Claude Code's built-in browser pane or Claude in Chrome, started against the dev server with `preview_start` and a `.claude/launch.json` entry) or a Playwright script; CDP for Electron; a PTY harness for CLI/TUI; plain HTTP for services. Name the exact tools the generated skill should call.
...
Tell the user the feature map has to track the app: when a feature changes, the agent that changed it updates its map file, and a periodic pass re-drives every mapped feature and fixes stale steps. pstack's `maintain-verification-skill` automates that pass if they want it later. Suggest a cadence only if they ask.
